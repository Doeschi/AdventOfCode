...
void Day08::solvePartOne() {
    std::unordered_set<Vector2D, Vector2DHash> uniqueLocations;
...
void Day08::initFrequencies() {
    for (auto y{0}; y < m_inputLines.size(); ++y) {
        ...
        for (auto x{0}; x < line.size(); ++x) {
            ...
            if (c != '.')
                m_frequencies[c].push_back(Vector2D{x, y});
        }
    }
}

void Day08::getAntiNodesFromAntennas(const std::vector<Vector2D>& antennas, const int maxIterations,
...
void Day08::getAntiNodes(const Vector2D& pos, const int xDiff, const int yDiff, const int maxIterations,
                         std::unordered_set<Vector2D, Vector2DHash>& uniqueLocations) const {
    for (int i = 1; i < maxIterations + 1; ++i) {
        Vector2D location{pos.x + (xDiff * i), pos.y + (yDiff * i)};
